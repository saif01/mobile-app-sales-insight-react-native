...
const SALES_URL = WEB_VIEW_TOKEN
  ? `https://qsr.cpbfivestar.com/embed/sort-sale-summary?token=${encodeURIComponent(WEB_VIEW_TOKEN)}`
  : 'https://qsr.cpbfivestar.com/embed/sort-sale-summary';

export default function SalesScreen() {
  const router = useRouter();
  const { accessPermissions } = useAuth();
  const { showLoader, hideLoader } = useGlobalLoader();
  useScreenPreloader({ message: 'Loading sales...' });

  useEffect(() => {
    return () => hideLoader();
  }, [hideLoader]);

  if (!canAccessQsrRoute(accessPermissions, 'qsr-reports/sales')) {
  ...
  if (Platform.OS === 'web') {
    return (
      <View style={styles.container}>
        <AppBar
          showMenuButton
          onMenuButtonPress={() => router.push('/navigation-panel?active=qsr-reports/sales')}
          title="QSR Sales"
          subtitle="Reports"
          showBottomBorder
        ...
        <View style={styles.webFallback}>
          ...
          <ExternalLink href={SALES_URL}>
            <ThemedText type="link">Open sales report</ThemedText>
          </ExternalLink>
        </View>
      </View>
    );
